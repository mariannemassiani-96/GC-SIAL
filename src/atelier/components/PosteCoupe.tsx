...
import { patchCommandeModule, listCommandesGlobales, getProfileImages, logProductionEvent, type CommandeGlobale, type ModuleStatus } from '../../api';
...
/** Convert a CommandeGlobale into a local Commande shape for display */
function convertGlobalToLocal(g: CommandeGlobale): Commande {
  const cp = g.coupe_profiles as CoupeModuleData | undefined;
  const machines: Commande['machines'] = {};
  const fileNames = cp?.fileNames;
  for (const m of ['lmt65', 'dt', 'renfort'] as MachineName[]) {
    ...
    machines[m] = {
      ...
      importDate: g.updated_at || new Date().toISOString(),
      statut: 'imported',
  ...
  return {
    id: `global_${g.ref}`,
    ref: g.ref,
    chantier: g.chantier || g.client || '',
    date: g.date_creation?.slice(0, 10) || new Date().toISOString().slice(0, 10),
    machines,
    statut: 'envoyee',
    notes: '',
  };
...
export function PosteCoupe({ onBack, startAtelier }: Props) {
  ...
  // ── Phase 2-3: Also fetch commandes_globales and merge ──
  const [globalCommandes, setGlobalCommandes] = useState<Commande[]>([]);

  useEffect(() => {
    listCommandesGlobales().then(globals => {
      const converted = globals
        .filter(g => g.coupe_profiles && typeof g.coupe_profiles === 'object' && (g.coupe_profiles as CoupeModuleData).machines)
        .map(g => convertGlobalToLocal(g));
      setGlobalCommandes(converted);
    }).catch(() => {});
  }, []);

  // Merge: local commandes take precedence over globals with the same ref
  const allCommandes = [...commandes, ...globalCommandes.filter(g => !commandes.some(c => c.ref === g.ref))];

  const selected = allCommandes.find(c => c.id === selectedId) ?? null;
  ...
  const updateCutStatut = useCallback((commandeId: string, machine: MachineName, barId: string, cutIdx: number, statut: FstCut['statut']) => {
    ...
    upsert(updated);
    syncCoupeToGlobal(updated);
    const cut = md.fstlineJob.bars.find(b => b.id === barId)?.cuts[cutIdx];
    logProductionEvent({ commande_ref: c.ref, poste: machine, action: statut, piece_ref: cut?.bcod || `bar${barId}_cut${cutIdx}` }).catch(() => {});
  ...
  const markBarAllCut = useCallback((commandeId: string, machine: MachineName, barId: string) => {
    ...
    // Sync to global dashboard (fire-and-forget)
    syncCoupeToGlobal(updated);
    // Log production events for each cut marked
    const bar = md.fstlineJob.bars.find(b => b.id === barId);
    if (bar) {
      for (const cut of bar.cuts) {
        if (cut.statut === 'a_couper') {
          logProductionEvent({ commande_ref: c.ref, poste: machine, action: 'coupe', piece_ref: cut.ref || cut.bcod || '' }).catch(() => {});
        }
      }
    }
  }, [commandes, upsert, user]);
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 max-w-5xl mx-auto w-full space-y-4">
        ...
        <div className="bg-indigo-600/10 border border-indigo-500/30 rounded-lg p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-indigo-300">
            Commandes aussi disponibles depuis le <span className="font-semibold">Tableau de Bord</span>. Les commandes avec fichiers FSTLINE importes apparaissent automatiquement ici.
          </p>
        </div>
